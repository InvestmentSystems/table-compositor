...
class TestUnit(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.multi_df = pd.DataFrame(data=dict(
            a=[0.1, 0.2, 0.3, 0.4],
            b=[100, 200, 300, 100],
            c=[True, False, True, False]))
        cls.multi_df.index = pd.MultiIndex.from_tuples([('a', 1), ('a', 2), ('b', 1), ('b', 2)])
        ...
        cls.multi_df_1 = pd.DataFrame(data=dict(
            a=[0.1, 0.2, 0.3, 0.4],
            b=[100, 200, 300, 100],
            c=[True, False, True, False],
            d=[10, 20, 40, 30]))
    ...
    def test_index_node_construction_simple_index(self):
        ...
        self.assertEqual([c.value for c in root.children],
                          ['a', 'b', 'c'])
        self.assertEqual([c.parent for c in root.children],
                          [root, root, root])
        self.assertEqual([c.key for c in root.children],
                         [('a',), ('b',), ('c',)])
    ...
    def test_index_node_construction_multi_hierarchical_columns(self):
        ...
        self.assertEqual([c.value for c in root.children],
                          ['a', 'b'])
        self.assertEqual([c.value for c in root.children[0].children],
                          [1, 2])
        self.assertEqual([c.value for c in root.children[1].children],
                          [1])
        self.assertEqual([c.parent for c in root.children],
                          [root, root])
        self.assertEqual([c.parent for c in root.children[0].children],
                          [root.children[0], root.children[0]])
        self.assertEqual([c.parent for c in root.children[1].children],
                          [root.children[1]])
        self.assertEqual([c.key for c in root.children],
                         [('a',), ('b',)])
        self.assertEqual([c.key for c in root.children[0].children],
                          [('a', 1), ('a', 2)])
        self.assertEqual([c.key for c in root.children[1].children],
                          [('b', 1)])
    ...
    def test_index_node_resolve_loc_simple_index(self):
        ...
        locs = pdpr.IndexNode.resolve_loc_vertical(
            root, (2, 3, 2, 3), row_hts=row_hts)
    ...
    def test_index_node_resolve_loc_multi_hierarchical_column(self):
        ...
        locs = pdpr.IndexNode.resolve_loc(
            root, (2, 3, 2, 3), col_widths=col_widths)        # first level

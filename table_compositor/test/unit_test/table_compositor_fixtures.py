...
from table_compositor.xlsx_writer import XlsxWriterCompositor
...
def get_scenarios():
    ...
    fixture_funcs = (
            get_simple_df_with_layout,
            get_multi_hierarchical_df_with_layouts)
    ...
    for p in permutations:
        fixture_name = 'test_{}_grid_{}_nested_{}_orientation_{}_{}'.format(
        ...
        fixture = TestFixtures(
                ...
                nested=p[2],
                orientation=p[3],
                engine_and_callback_funcs_cls=p[4])
...
class HtmlCallBackFunc:
    ...
    def data_value_func(self, r, c):
        return df_type_to_str(self.df.at[r, c])

    def data_style_func(self, r, c):
        if isinstance(self.df.at[r, c], (np.int_, np.float, np.uint)):
    ...
    def header_style_func(self, node):
        return dict(text_align='center', background_color='#4F81BD', )
...
def get_simple_df_with_layout(grid=False, nested=False, callback_func_cls=XlsxCallBackFunc):
...
def get_multi_hierarchical_df_with_layouts(grid=True, nested=False, callback_func_cls=XlsxCallBackFunc):

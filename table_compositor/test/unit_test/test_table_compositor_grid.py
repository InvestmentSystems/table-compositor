...
class LayoutFixtures:
    ...
    def get_all_fixtures():
        ...
        fixture0 = (
            [pm22, pm22, pm22],
            ((0, 0), (3, 0), (6, 0))) # assuming 1 row padding
        fixture1 = (
            [pm42, [pm22, pm32]],
            ((0, 0), (5, 0), (5, 3)))
    ...
    def  __new__(cls, names, bases, attrs):
        def test_func_constructor(fixtures, is_vertical):
            def _func(self):
                ...
                shift_func = (
                    lambda pm, i, j:
                    PM(pm.name, pm.i + i, pm.j + j, pm.max_rows, pm.max_cols))
                ht_func = lambda pm: pm.max_rows
                width_func = lambda pm: pm.max_cols

                grid = glm.GridLayoutManager.build_cells(
                    fixture, vertical=is_vertical)
        ...
        for i, fixtures in enumerate(LayoutFixtures.get_all_fixtures()):
            ...
            attrs[test_name] = test_func_constructor(
                fixtures, is_vertical=True)

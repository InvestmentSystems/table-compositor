...
def write_using_openpyxl(pm_openpyxl):
    ...
    tcew.XLSXWriter.to_xlsx(pm_openpyxl, output_fp, engine=tcew.SupportedEngines.openpyxl)

def write_using_xlsxwriter(pm_xlsxwriter):
    output_fp = os.path.join(tempfile.gettempdir(), 'xlsxwriter_test.xlsx')
    print('file://' + output_fp)
    tcew.XLSXWriter.to_xlsx(pm_xlsxwriter, output_fp, engine=tcew.SupportedEngines.xlsxwriter)
...
def time_xlsx_writing(scenarios: tp.Iterable[tp.Tuple[int, int]]):
    ...
    results['(xlsxwriter/openpyxl) (%)'] = (results['xlsxwriter'] / results['openpyxl']) * 100
...
def time_presentation_model(scenarios: tp.Iterable[tp.Tuple[int, int]]):
    ...
    results['(column_level/cell_level) (%)'] = (results['column_level'] / results['cell_level']) * 100
